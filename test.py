...
if __name__ == '__main__':
    ...
    _, _, input_test, gt_test = split_dataset(dataset, test_set=args.test, features=args.features)
    ...
    if args.model_type == "MLP":
        ...
        # model = VT.ViTRegression(in_channels=num_channels, patch_size=(3, 3), emb_dim=256, mlp_dim=512, device=device, load=True, model_name=args.model_name) # patch_size=(h, w)=(3, 3)
        # model = VT.ViTRegression(in_channels=num_channels, patch_size=(3, 3), emb_dim=256, mlp_dim=512, num_heads=8, device=device, load=True, model_name=args.model_name) # patch_size=(h, w)=(3, 3)

        model = VT.ViTRegression(in_channels=num_channels, patch_size=(3, 3), emb_dim=64, mlp_dim=128, num_heads=2, num_layers=2, device=device, load=True, model_name=args.model_name)
